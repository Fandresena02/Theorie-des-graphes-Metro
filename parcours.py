        ...
        for sommet in voisins_sommets_tries:
            ...
            if sommet == station_courante[0]:
                for voisin in list(voisins_sommets_tries.values())[sommet]:
                    addition_durees = durees_min[sommet] + \
                        list(voisins_sommets_tries.values())[sommet][voisin]
...
def get_ligne_lien(station1, station2):
    ...
    for station in stations:
        if station['nom_sommet'] == station1['nom_sommet']:
            stations1.append(station)
        if station['nom_sommet'] == station2['nom_sommet']:
...
def parcours_chemin(durees_min, peres, depart, arrivee):
    ...
    for station in stations:
        if station['nom_sommet'] == depart:
            num_dep = station['numero_sommet']
        if station['nom_sommet'] == arrivee:
            num_arr = station['numero_sommet']
    ...
    for station in reversed(itineraire):
        ...
        # détermination de la ligne
            ...
            print('Prenez la ligne', ligne, 'en direction de',
                  get_direction(station_precedente, station, ligne))

        # indication de changement de ligne
        if changement:
            ligne = get_ligne_lien(station_precedente, station)
            print('A', station_precedente['nom_sommet'], 'changez et prenez la ligne',
                  station_precedente['ligne'], 'en direction de', get_direction(station_precedente, station, ligne))
            changement = False
        ...
        if station['numero_sommet'] == num_arr:
            print('Vous devriez arriver à', station['nom_sommet'], 'dans environ', round(
                (durees_min[station['numero_sommet']]/60), 2), 'minutes')
